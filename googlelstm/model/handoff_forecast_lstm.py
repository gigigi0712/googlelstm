...
class HandoffForecastLSTM(BaseModel):
    ...
    module_parts = ['hindcast_embedding_net', 'forecast_embedding_net', 'hindcast_lstm', 'forecast_lstm',
                    'hindcast_head', 'forecast_head', 'handoff_net']
    ...
    def forward(self, data: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # first_row = x_f[0, :, :]  # shape: [batchsize, forcast_seq_length * len(hindcast_inputs)]
        #
        # import pdb;pdb.set_trace()  # 这里会暂停代码
        # x_f = first_row.reshape(self.cfg.forecast_seq_length, -1, len(self.cfg.hindcast_inputs))  # shape: [6, 1, 5]

        # run the hindcast lstm
        lstm_output_hindcast, (h_n_hindcast, c_n_hindcast) = self.hindcast_lstm(
            x_h[:self.initial_hindcast_seq_length, ...])
        lstm_output_hindcast = lstm_output_hindcast.transpose(0, 1)
        if x_h.shape[0] > self.initial_hindcast_seq_length:
            lstm_output_hindcast_overlap, _ = self.hindcast_lstm(x_h[self.initial_hindcast_seq_length:, ...],
                                                                 (h_n_hindcast, c_n_hindcast))
        ...
        if x_h.shape[0] > self.initial_hindcast_seq_length:
            ...
            pred = {key: torch.cat([y_hindcast[key], y_hindcast_overlap[key], y_forecast[key]], dim=1) for key in
                    y_hindcast}
            # import pdb; pdb.set_trace()  # 这里会暂停代码
        else:
