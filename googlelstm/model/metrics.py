...
def get_sim_peak_times(
    ...
    """
    Extract the peak times from the simulated data, ensuring peaks are identified in sim itself.

    Parameters
    ----------
    obs: DataArray
        Observed time series data (used for masking invalid data).
    sim: DataArray
    ...
    -------
    list
        A list of peak times from the simulated data.
    """
    # Verify inputs
    _validate_inputs(obs, sim)

    # Remove NaN values and mask valid data
    obs, sim = _mask_valid(obs, sim)

    # Infer datetime coordinate if not provided
    if datetime_coord is None:
        datetime_coord = utils.infer_datetime_coord(sim)

    # Automatically infer window size if not provided
    if window is None:
        window = max(int(utils.get_frequency_factor('12h', resolution)), 3)

    # Find peaks in the simulated data itself
    peaks, _ = signal.find_peaks(sim.values, distance=100, prominence=np.std(sim.values))

    # Extract the times of the peaks
    sim_peak_times = [sim[datetime_coord].values[idx] for idx in peaks]
