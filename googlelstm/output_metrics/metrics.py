...
def get_available_metrics() -> List[str]:
    ...
    metrics = [
        ...
        "Peak-Timing", "RE-PEAK-Timing","RE-PEAK-MAPE", "Peak-MAPE", "accp10", "accp20","my_rmse","my_mae","my_mape"
    ]
    return metrics
...
def my_rmse(obs: DataArray, sim: DataArray) -> float:
...
def my_mae(obs: DataArray, sim: DataArray) -> float:
    r"""Calculate mean absolute error (MAE).

    .. math:: \text{MAE} = \frac{1}{T}\sum_{t=1}^T |\widehat{y}_t - y_t|,
    ...
    return float(abs(sim - obs).mean())  # 使用 Python 内置 abs() 函数


def my_mape(obs: DataArray, sim: DataArray) -> float:
    r"""Calculate mean absolute percentage error (MAPE).

    .. math:: \text{MAPE} = \frac{100}{T}\sum_{t=1}^T \left|\frac{\widehat{y}_t - y_t}{y_t}\right|,
    ...
    Notes
    -----
    Observed values (`obs`) must not contain zeros, as division by zero is undefined.
    ...
    # Mask zero values in obs
    non_zero_mask = obs != 0
    obs = obs[non_zero_mask]
    sim = sim[non_zero_mask]

    return float((abs(sim - obs) / obs).mean() * 100)

def accp10(obs: DataArray, sim: DataArray) -> float:
    r"""Calculate ACCP10 (Absolute Correctness within 10%).

    ACCP10 measures the proportion of predictions (sim) where the absolute difference between the predicted
    value and the observed value is within 10% of the observed value's absolute value.

    .. math:: \text{ACCP10} = \frac{\sum_{t=1}^{T} \mathbb{1}\left(\frac{|Q_m^t - Q_o^t|}{|Q_o^t|} \leq 0.1 \right)}{T},

    where :math:`Q_m` are the simulations (here, `sim`), :math:`Q_o` are observations (here, `obs`),
    and :math:`\mathbb{1}` is the indicator function that returns 1 if the condition is met, otherwise 0.
    ...
    -------
    float
        ACCP10 value, which is the fraction of predictions within 10% error of the observed values.

    Notes
    -----
    This metric is bounded between 0 and 1:
    - **1**: All predictions are within 10% of the observed values.
    - **0**: None of the predictions are within 10% of the observed values.
    """
    ...
    absolute_percentage_error = abs(sim - obs) / abs(obs)

    # calculate the proportion of errors within 10%
    within_10_percent = (absolute_percentage_error <= 0.1).sum()

    # calculate ACCP10 as the proportion of valid predictions
    accp10_value = within_10_percent / len(obs)

    return float(accp10_value)


def accp20(obs: DataArray, sim: DataArray) -> float:
    r"""Calculate ACCP10 (Absolute Correctness within 10%).

    ACCP20 measures the proportion of predictions (sim) where the absolute difference between the predicted
    value and the observed value is within 10% of the observed value's absolute value.
    ...
    -------
    float
        ACCP20 value, which is the fraction of predictions within 10% error of the observed values.
    ...
    # calculate the proportion of errors within 10%
    within_20_percent = (absolute_percentage_error <= 0.2).sum()

    # calculate ACCP10 as the proportion of valid predictions
    accp20_value = within_20_percent / len(obs)

    return float(accp20_value)
...
def mean_peak_timing(obs: DataArray,
    ...
    if window is None:
        ...
        window = max(int(utils.get_frequency_factor('12h', resolution)), 6)
...
def refine_mean_peak_timing(
    ...
    for idx in peaks:
        ...
        print(peak_obs.coords[datetime_coord])
        print(peak_sim.coords[datetime_coord])
...
def get_obs_peak_values(
    ...
    for idx in peaks:
        ...
        peak_error = np.abs(peak_obs.data - peak_sim.data) / (peak_obs.data + 1e-8)
        peak_obs_values.append(peak_obs.data)

    return peak_obs_values

def get_sim_peak_values(
    obs: DataArray,
    sim: DataArray,
    window: int = None,
    resolution: str = '1D',
    ...
    for idx in peaks:
        ...
        # get xarray object of qobs peak, for getting the date and calculating the datetime offset
        peak_obs = obs[idx]
        peak_sim_values.append(peak_sim.data)

    return peak_sim_values
...
def get_peak_times(
    ...
    """
    Extract the peak times from the observed data.
    ...
    window: int, optional
        Window size for searching peaks. If None, it will be inferred.
    resolution: str, optional
        Time resolution (e.g., '1D', '12H').
    datetime_coord: str, optional
        Name of the datetime coordinate in the DataArray. If None, inferred automatically.

    Returns
    -------
    list
        A list of peak times from the observed data.
    """
    # Verify inputs
    if not isinstance(obs, DataArray):
        raise ValueError("Input 'obs' must be an xarray.DataArray.")
    if datetime_coord is None:
        # Try to infer the datetime coordinate automatically
        datetime_coord = obs.dims[0]

    # Remove NaN values from the observed data
    obs = obs.dropna(dim=datetime_coord)

    # Find peaks in the observed data
    peaks, _ = signal.find_peaks(obs.values, distance=distance, prominence=np.std(obs.values))

    # Extract the times of the peaks
    peak_times = [obs[datetime_coord].values[idx] for idx in peaks]

    return peak_times

def get_sim_peak_times(
    obs: DataArray,
    sim: DataArray,
    window: int = None,
    resolution: str = '1D',
    ...
    """
    Extract simulated peak times based on observed peak locations.
    ...
    sim : DataArray
        Simulated time series.
    window : int, optional
        Window size for searching peaks in the simulation.
    resolution : str, optional
        Temporal resolution, e.g., '1D' for daily.
    datetime_coord : str, optional
        Name of the datetime coordinate.

    Returns
    -------
    list
        A list of simulated peak times.
    """
    # Validate inputs
    if not isinstance(obs, DataArray) or not isinstance(sim, DataArray):
        raise ValueError("Both 'obs' and 'sim' must be xarray.DataArray objects.")
    if len(obs) != len(sim):
        raise ValueError("Observed and simulated data must have the same length.")
    if datetime_coord is None:
        # Infer datetime coordinate from the first dimension
        datetime_coord = obs.dims[0]

    # Remove NaN values from the observed and simulated data
    obs, sim = obs.dropna(dim=datetime_coord), sim.dropna(dim=datetime_coord)

    # Heuristic to get indices of peaks in observed data
    peaks, _ = signal.find_peaks(obs.values, distance=distance, prominence=np.std(obs.values))

    # Infer a reasonable window size if not specified
    if window is None:
        window = max(int(12 / pd.to_timedelta(resolution).total_seconds() * 3600), 7)

    # Extract simulated peak times
    sim_peak_times = []
    for idx in peaks:
        # Skip peaks at the edges or around NaNs
        if (idx - window < 0) or (idx + window >= len(obs)):
            continue

        # Check if the value at idx is a peak in sim
        if (sim[idx] > sim[idx - 1]) and (sim[idx] > sim[idx + 1]):
            peak_sim = sim[idx]
        else:
            # Get the window of sim values
            window_values = sim[idx - window: idx + window + 1]

            # Find the maximum value and retain full xarray object
            peak_sim = window_values.where(window_values == window_values.max(), drop=True)[0]

        # Append the time coordinate of the simulated peak
        sim_peak_times.append(peak_sim.coords[datetime_coord].values)

    return sim_peak_times

def get_sim_times_f1score(
    sim: DataArray,
    window: int = None,
    resolution: str = '1D',
    datetime_coord: str = None
    ...
    """
    # Verify inputs
    if not isinstance(sim, DataArray):
        raise ValueError("Input 'obs' must be an xarray.DataArray.")
    if datetime_coord is None:
        # Try to infer the datetime coordinate automatically
        datetime_coord = sim.dims[0]

    # Remove NaN values from the observed data
    sim = sim.dropna(dim=datetime_coord)

    # Find peaks in the observed data
    peaks, _ = signal.find_peaks(sim.values, distance=distance, prominence=np.std(sim.values))

    # Extract the times of the peaks
    sim_peak_times_f1score = [sim[datetime_coord].values[idx] for idx in peaks]

    return sim_peak_times_f1score
...
def calculate_all_metrics(obs: DataArray,
    ...
    results = {
        "acc10": accp10(obs, sim),
        "acc20": accp20(obs, sim),
        "NSE": nse(obs, sim),
        "MSE": mse(obs, sim),
        "RMSE": rmse(obs, sim),
        "KGE": kge(obs, sim),
        ...
        "Re-Peak-Timing": refine_mean_peak_timing(obs, sim, resolution=resolution, datetime_coord=datetime_coord),
        "Re-Peak-MAPE": refine_mean_peak_mape(obs, sim, resolution=resolution, datetime_coord=datetime_coord),
        "Get-Peak-Timing": get_peak_times(obs, sim, resolution=resolution, datetime_coord=datetime_coord),
        "Get-Sim-Peak-Timing": get_sim_peak_times(obs, sim, resolution=resolution, datetime_coord=datetime_coord),
        "Get-Sim-Peak-Timing-F1score": get_sim_times_f1score(sim, resolution=resolution, datetime_coord=datetime_coord),
        "Get-Obs-Peak-Value": get_obs_peak_values(obs, sim, resolution=resolution, datetime_coord=datetime_coord),
        "Get-Sim-Peak-Value": get_sim_peak_values(obs, sim, resolution=resolution, datetime_coord=datetime_coord)
    }
...
def calculate_metrics(obs: DataArray,
    ...
    for metric in metrics:
        ...
        elif metric.lower() == "my_rmse":
            values["my_rmse"] = my_rmse(obs, sim)#"my_rmse","my_mae","my_mape"
        elif metric.lower() == "my_mae":
            values["my_mae"] = my_mae(obs, sim)
        elif metric.lower() == "my_mape":
            values["my_mape"] = my_mape(obs, sim)
        elif metric.lower() == "accp10":
            values["accp10"] = accp10(obs, sim)
        elif metric.lower() == "accp20":
            values["accp20"] = accp20(obs, sim)
        ...
        elif metric.lower() == "re-peak-timing":
            values["RE-Peak-Timing"]= refine_mean_peak_timing(obs, sim, resolution=resolution, datetime_coord=datetime_coord)
        elif metric.lower() == "re-peak-mape":
            values["RE-Peak-MAPE"] = refine_mean_peak_mape(obs, sim, resolution=resolution,
                                                               datetime_coord=datetime_coord)
        elif metric.lower() == "get-peak-timing":
            values["Get-Peak-Timing"] = get_peak_times(obs, sim, resolution=resolution, datetime_coord=datetime_coord)
        elif metric.lower() == "get-sim-peak-timing":
            values["Get-Sim-Peak-Timing"] = get_sim_peak_times(obs, sim, resolution=resolution, datetime_coord=datetime_coord)
        elif metric.lower() == "get-sim-peak-timing-f1score":
            values["Get-Sim-Peak-Timing-F1score"] = get_sim_times_f1score(sim, resolution=resolution, datetime_coord=datetime_coord)
        elif metric.lower() == "get-obs-peak-value":
            values["Get-obs-Peak-Value"] = get_obs_peak_values(obs, sim, resolution=resolution,
                                                               datetime_coord=datetime_coord)
        elif metric.lower() == "get-sim-peak-value":
            values["Get-Sim-Peak-Value"] = get_sim_peak_values(obs, sim, resolution=resolution,
                                                               datetime_coord=datetime_coord)
        else:
