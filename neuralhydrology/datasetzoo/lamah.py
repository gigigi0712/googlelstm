...
def load_lamah_forcing(data_dir: Path, basin: str, sub_dataset: str, temporal_resolution: str = '1D') -> pd.DataFrame:
    ...
    temporal_resolution_directory = 'daily_ours' if temporal_resolution == '1D' else 'hourly'
...
def load_lamah_discharge(data_dir: Path,
    ...
    temporal_resolution_directory = '24hourly_for_era5' if temporal_resolution == '1D' else 'hourly'
...
def load_lamah_attributes(data_dir: Path, sub_dataset: str, basins: List[str] = []) -> pd.DataFrame:
    ...
    file_path = data_dir / _SUBDATASET_TO_DIRECTORY[sub_dataset]  / "1_attributes" / "danube_358_station_allbasin_static_attributes.csv"
    df_caravan = _load_lamah_attribute_csv_file(file_path)
    ...
    df = pd.concat([df_catchment, df_gauge, df_caravan], axis=1)
