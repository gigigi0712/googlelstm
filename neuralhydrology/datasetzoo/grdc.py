...
class GRDC(BaseDataset):
    ...
    def __init__(self,
        ...
        super(GRDC, self).__init__(cfg=cfg,
                                      is_train=is_train,
                                      period=period,
                                      basin=basin,
    ...
    def _load_basin_data(self, basin: str) -> pd.DataFrame:
        ...
        return load_grdc_timeseries(data_dir=self.cfg.data_dir, basin=basin)

    def _load_attributes(self) -> pd.DataFrame:
        """Load input and output data from text files."""
        return load_grdc_attributes(data_dir=self.cfg.data_dir, basins=self.basins)


def load_grdc_attributes(data_dir: Path,
...
# 替换原始匹配代码,不区分大小写
    if basins:
        # 创建大小写无关的索引映射
        index_lower = [idx.lower() for idx in df.index]
        basins_lower = [b.lower() for b in basins]

        # 检查所有 basins 是否在 df.index 中（忽略大小写）
        if any(b not in index_lower for b in basins_lower):
            raise ValueError('Some basins are missing static attributes.')

        # 筛选出对应的行
        matched_indices = [df.index[i] for i, idx_lower in enumerate(index_lower) if idx_lower in basins_lower]
        df = df.loc[matched_indices]

    return df


def load_grdc_timeseries(data_dir: Path, basin: str, filetype: str = "csv") -> pd.DataFrame:
    ...
    # if subdataset_name == "danube":
    attributes = load_grdc_attributes(data_dir, basins=[basin], subdataset=subdataset_name)
    area = attributes.loc[basin, 'area']
    df["qobs"] = _normalize_discharge(df["qobs"], area)
