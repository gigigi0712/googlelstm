...
class BaseDataset(Dataset):
    ...
    def _load_combined_attributes(self):
        ...
        if self.cfg.static_attributes:
            ...
            #        'zero_q_freq', 'hfd_mean', 'gauge_name', 'soil_depth_pelletier',
        ...
        if dfs:
            ...
            # 加這四行
            attribute_stds_safe = self.scaler["attribute_stds"].copy(deep=True)
            attribute_stds_safe = attribute_stds_safe.where(
                attribute_stds_safe > 1e-10,  # 或寫成 scale_safe != 0 也可以，大多數情況夠用
                1.0
            )
            ...
            if any([k.startswith("camels_attr") for k in self.scaler.keys()]):
                ...
                df = (df - self.scaler['attribute_means']) / attribute_stds_safe
            ...
            # 修改不区分大小写
            for basin in self.basins:
                basin_lower = basin.lower()  # 转换为小写
                df.index = df.index.str.lower()  # 统一 DataFrame 索引的小写
                attributes = df.loc[df.index == basin_lower].values.flatten()
                self._attributes[basin] = torch.from_numpy(attributes.astype(np.float32))
    ...
    def _load_data(self):
        ...
        # 加這四行
        scale_safe = self.scaler["xarray_feature_scale"].copy(deep=True)
        scale_safe = scale_safe.where(
            scale_safe > 1e-10,  # 或寫成 scale_safe != 0 也可以，大多數情況夠用
            1.0
        )
        # import pdb;
        # pdb.set_trace()  # 这里会暂停代码

        xr = (xr - self.scaler["xarray_feature_center"]) / scale_safe
    ...
    def _setup_normalization(self, xr: xarray.Dataset):
        ...
        # for suf in suffixes:
        #     var_name = f"t2m_{suf}"
        #     if var_name in self.scaler["xarray_feature_scale"]:
        #         self.scaler["xarray_feature_scale"][var_name] = self.scaler["xarray_feature_scale"][t2m_source]
        #     if var_name in self.scaler["xarray_feature_center"]:
        #         self.scaler["xarray_feature_center"][var_name] = self.scaler["xarray_feature_center"][t2m_source]
        #
        # # 替换 tp 相关字段
        # for suf in suffixes:
        #     var_name = f"tp_{suf}"
        #     if var_name in self.scaler["xarray_feature_scale"]:
        #         self.scaler["xarray_feature_scale"][var_name] = self.scaler["xarray_feature_scale"][tp_source]
        #     if var_name in self.scaler["xarray_feature_center"]:
        #         self.scaler["xarray_feature_center"][var_name] = self.scaler["xarray_feature_center"][tp_source]
        ...
        suffixes = ["L1", "L2", "L3", "L4", "L5", "L6", "L7"]

        for var, source in source_map.items():
            for suf in suffixes:
                var_name = f"{var}_{suf}"

                if var_name in self.scaler["xarray_feature_scale"]:
                    self.scaler["xarray_feature_scale"][var_name] = \
                        self.scaler["xarray_feature_scale"][source]

                if var_name in self.scaler["xarray_feature_center"]:
                    self.scaler["xarray_feature_center"][var_name] = \
                        self.scaler["xarray_feature_center"][source]
    ...
    #         else:
    #             tensor_data = torch.as_tensor(batch[key])
    #             batch[key] = tensor_data.pin_memory() if tensor_data.device.type == 'cpu' else tensor_data
    #
    #     return batch
