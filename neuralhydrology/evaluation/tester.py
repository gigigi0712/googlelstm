...
from googlelstm.model.metrics import calculate_metrics, get_available_metrics
...
class BaseTester(object):
            ...
            for freq in ds.frequencies:
                ...
                if metrics:
                    for target_variable in self.cfg.target_variables:
                        ...
                        # # 修改铆钉步长，改回h0
                        obs = xr.isel(time_step=slice(-self.cfg.predict_last_n, -self.cfg.predict_last_n+3)) \
                            .stack(datetime=['date', 'time_step']) \
                            .drop_vars({'datetime', 'date', 'time_step'})[f"{target_variable}_obs"]

                        # # stack dates and time_steps so we don't just evaluate every 24H when use_frequencies=[1D, 1H]
                        # obs = xr.isel(time_step=slice(-self.cfg.predict_last_n, 0)) \
                        #     .stack(datetime=['date', 'time_step']) \
                        #     .drop_vars({'datetime', 'date', 'time_step'})[f"{target_variable}_obs"]
                        # import pdb;pdb.set_trace()  # 这里会暂停代码
                        n = len(freq_date_range)

                        datetime_5098 = pd.date_range(
                            start=freq_date_range[0],
                            periods=n * 3,
                            freq="D"
                        )

                        obs['datetime'] = datetime_5098


                        # check if there are observations for this period
                        if not all(obs.isnull()):
                            sim = xr.isel(time_step=slice(-self.cfg.predict_last_n, -self.cfg.predict_last_n+3)) \
                                .stack(datetime=['date', 'time_step']) \
                                .drop_vars({'datetime', 'date', 'time_step'})[f"{target_variable}_sim"]
                            # sim = xr.isel(time_step=slice(-self.cfg.predict_last_n, 0)) \
                            #     .stack(datetime=['date', 'time_step']) \
                            #     .drop_vars({'datetime', 'date', 'time_step'})[f"{target_variable}_sim"]
                            sim['datetime'] = datetime_5098
                ...
                # if metrics:
                #     for target_variable in self.cfg.target_variables:
                #         # 建议先取出来，避免每次都写 self.cfg.predict_last_n（尤其当它是 dict 时）
                #         n_pred = self.cfg.predict_last_n
                #         if isinstance(n_pred, dict):
                #             n_pred = self.cfg.predict_last_n.get(freq, 0)  # 多频率时按 freq 取
                #
                #         if n_pred <= 0:
                #             continue  # 这个频率不预测，直接跳过
                #
                #         # ------------------- 观测值 -------------------
                #         obs = (
                #             xr[f"{target_variable}_obs"]  # 直接从 Dataset 取变量，更安全
                #                 .isel(time_step=slice(-n_pred, None))  # 最后 n_pred 步
                #                 .stack(datetime=["date", "time_step"])
                #                 .reset_index(["date", "time_step"], drop=True)  # 替代 drop_vars，更推荐
                #         )
                #
                #         # ------------------- 模拟值 -------------------
                #         sim = (
                #             xr[f"{target_variable}_sim"]
                #                 .isel(time_step=slice(-n_pred, None))
                #                 .stack(datetime=["date", "time_step"])
                #                 .reset_index(["date", "time_step"], drop=True)
                #         )
                #
                #         # ──────────────── 这里加这几行 ────────────────
                #         # 把 freq_date_range 重复 n_pred 次，变成完整的时间序列
                #         freq_date_range_full = np.tile(freq_date_range.values, n_pred)
                #         # 或者用 pandas 写法（更稳，推荐）：
                #         # freq_date_range_full = pd.concat([freq_date_range] * n_pred).sort_values()
                #
                #         # 长度检查（调试用，可选，但强烈建议保留）
                #         expected_len = len(xr['date']) * n_pred
                #         actual_len = len(freq_date_range_full)
                #         if actual_len != expected_len:
                #             print(f"Warning: datetime length mismatch! Expected {expected_len}, got {actual_len}")
                #             continue  # 或 raise 看情况
                #
                #         # ──────────────── 然后用扩展后的版本赋值 ────────────────
                #         obs['datetime'] = freq_date_range_full
                #         sim['datetime'] = freq_date_range_full
                #         # check if there are observations for this period
                #         if not all(obs.isnull()):
                #             # sim = xr.isel(time_step=slice(-self.cfg.predict_last_n, None)) \
                #             #     .stack(datetime=['date', 'time_step']) \
                #             #     .drop_vars({'datetime', 'date', 'time_step'})[f"{target_variable}_sim"]
    ...
    def _save_results(self, results: Optional[dict], states: Optional[dict] = None, epoch: int = None):
        ...
        if self.cfg.metrics and results is not None:
            ...
            # 添加 mean 和 median 到原始数据
            df_all = df.copy()
            df_all.loc['mean'] = df_all.mean(numeric_only=True)
            df_all.loc['median'] = df_all.median(numeric_only=True)

            # 保存完整数据
            all_metrics_file = parent_directory / f"{self.period}_all_metrics.csv"
            df_all.to_csv(all_metrics_file)

            # 过滤 NSE > 0 的数据（排除 mean/median）
            df_filtered = df[df['NSE'] > 0].copy()
            df_filtered.loc['mean'] = df_filtered.mean(numeric_only=True)
            df_filtered.loc['median'] = df_filtered.median(numeric_only=True)

            filtered_metrics_file = parent_directory / f"{self.period}_filtered_NSE_gt_0.csv"
            df_filtered.to_csv(filtered_metrics_file)

            LOGGER.info(f"Stored full metrics at {all_metrics_file}")
            LOGGER.info(f"Stored filtered metrics (NSE>0) at {filtered_metrics_file}")

            # ===== 保存 RE-Peak-Timing 和 RE-Peak-MAPE 到 Excel =====
            re_peak_rows = []
            for basin, resolutions in results.items():
                for res, metrics in resolutions.items():
                    re_peak_timing = metrics.get("RE-Peak-Timing", [])
                    re_peak_mape = metrics.get("RE-Peak-MAPE", [])

                    if isinstance(re_peak_timing, list) and isinstance(re_peak_mape, list) and len(
                            re_peak_timing) == len(re_peak_mape):
                        for i, (t, m) in enumerate(zip(re_peak_timing, re_peak_mape)):
                            re_peak_rows.append({
                                "Basin": basin,
                                "Index": i,
                                "RE-Peak-Timing": t,
            ...
            # 统计 NSE > 0 的数量（不包括 mean/median）
            nse_valid_count = (df['NSE'] > 0).sum()

            # 创建 RE-Peak-Details 的 DataFrame
            if re_peak_rows:
                re_peak_df = pd.DataFrame(re_peak_rows)

                # ===== 新增统计逻辑 =====
                # 统计所有峰值的数量
                total_peak_count = len(re_peak_df)

                # 统计 RE-Peak-Timing >= 1 的数量
                re_peak_timing_ge_1_count = (re_peak_df["RE-Peak-Timing"] < 1).sum()

                # 对 RE-Peak-Timing < 1 的数据，计算绝对值平均值
                re_peak_timing_lt_1 = re_peak_df[re_peak_df["RE-Peak-Timing"] < 1]
                mean_abs_re_peak_timing_lt_1 = re_peak_timing_lt_1["RE-Peak-Timing"].abs().mean()

                # 计算对应 RE-Peak-Timing < 1 的 RE-Peak-MAPE 平均值
                mean_re_peak_mape_lt_1 = re_peak_timing_lt_1["RE-Peak-MAPE"].mean()
                ...
                summary_df = pd.DataFrame({
                    "Metric": [
                        "NSE>0 Count",
                        "Total Peak Count",
                        "RE-Peak-Timing < 1 Count",
                        "Mean Absolute RE-Peak-Timing < 1",
                        "Mean RE-Peak-MAPE (RE-Peak-Timing < 1)"
                    ],
                    "Value": [
                        nse_valid_count,
                        total_peak_count,
                        re_peak_timing_ge_1_count,
                        mean_abs_re_peak_timing_lt_1,
                        mean_re_peak_mape_lt_1
                    ]
                })

                # 保存到 Excel
                from pandas import ExcelWriter
                excel_path = parent_directory / f"{self.period}_peak_details.xlsx"
                with ExcelWriter(excel_path) as writer:
                    df_all.to_excel(writer, sheet_name="Metrics", index=True)
                    re_peak_df.to_excel(writer, sheet_name="RE-Peak-Details", index=False)
                    summary_df.to_excel(writer, sheet_name="Summary", index=False)

                LOGGER.info(f"Stored detailed RE-peak metrics and summary at {excel_path}")
        ...
        with torch.no_grad():
            for data in loader:
                ...
                for freq in frequencies:
                    ...
                    # freq_key = f'_{freq}' if len(frequencies) == 1 else ''
                    ...
                    date_sub = data[f'date'][:, -predict_last_n[freq]:]
