...
class BaseTrainer(object):
    ...
    def initialize_training(self):
        ...
        if self.cfg.checkpoint_path is not None:
            ...
            # checkpoint_path = Path(self.cfg.base_run_dir)/ "model_epoch025.pt"
        ...
        # elif self.cfg.checkpoint_path is None and self.cfg.is_finetuning:
        #     base_path = Path(self.cfg.base_run_dir)
        #
        #     if base_path.is_file() and base_path.suffix == '.pt':
        #         # 情况一：base_run_dir 是一个 .pt 文件，直接用
        #         checkpoint_path = base_path
        #     elif base_path.is_dir():
        #         # 情况二：base_run_dir 是目录，从中找 model_epoch*.pt 文件
        #         checkpoint_path = [x for x in sorted(list(self.cfg.base_run_dir.glob('model_epoch*.pt')))][-1]
        #     else:
        #         raise ValueError(f"Invalid base_run_dir: {base_path} is neither a .pt file nor a directory")
        #
        #     LOGGER.info(f"Starting training from checkpoint {checkpoint_path}")
        #     self.model.load_state_dict(torch.load(str(checkpoint_path), map_location=self.device))
