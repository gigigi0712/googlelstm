...
def main():
    ...
    # 从文件中读取站点ID
    station_ids = read_station_ids("camelus_finetune.txt")  # 假设文件名为 "1_danube_basin.txt"

    # 创建存储图像的文件夹
    img_folder = Path("E:/DL/code/Neuralhydrology/neuralhydrology-modelzoo/googleLstmproject/results/Camelus_W365L1_531_runoff")
    ...
    # 路径设置
    run_dir = Path("runs/grdc_basins_W365L1_regression_runoff_finetune_exp1_0703_162902")
    eval_run(run_dir=run_dir, period="test", epoch=10)
    ...
    for station_id in station_ids:
        # 获取每个站点的观测数据和模拟数据
        try:
            qobs = results[station_id]['1D']['xr']['QObs(mm/d)_obs']
            qsim = results[station_id]['1D']['xr']['QObs(mm/d)_sim']
    ...
    # 保存为 Excel 文件
    accuracy_df.to_excel("E:/DL/code/Neuralhydrology/neuralhydrology-modelzoo/googleLstmproject/results/Camelus_finetune_runoff_exp1_all_accuracy_and_nse_results.xlsx", index=False)
